acumulado = 0
empleados = int(input("Cuantos empleados va a registrar? "))

for i in range (empleados):
    print(f"\n\tIngrese el {i+1}° empleado:\n")
    
    salario = int(input(f"Ingrese {i+1}° salario: "))
    ...
    if rendimiento > 90:
        print(f"Rendimiento excelente: {rendimiento:.0f}%")
    elif rendimiento >70 and rendimiento <=90:
        print(f"Rendimiento regular: {rendimiento:.0f}%")
    else:
        print(f"Rendimiento bajo: {rendimiento:.0f}%")

    acumulado += valor_pagar

print(f"\n\tEl valor acumulado es: ${acumulado:,.0f}")